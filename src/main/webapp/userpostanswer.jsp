							...
							<i class="bi bi-alexa"></i>
										...
										<%
											...
											%>
												...
												<input type="hidden" name="usernamet" value='<%=rsd.getString(9)%>'>
												<input type="hidden" name="useremail" value='<%=rsd.getString(11)%>'>
												<input type="hidden" name="nuserid" value='<%=rsd.getString(5)%>'>
								...
								<input type="hidden" value='<%=rsd.getString(1)%>' name="qid">
									...
									<textarea name="t2" class="form-control" placeholder="Write Your Answer..."></textarea>	
									<script>
					                        CKEDITOR.replace('t2');
					                </script>
					                <%
								  ...
								  {
									  %>
									  <span style="color:red">Please Enter Valid Answer.</span>
									  <% 
								  }
									%>
								</div>
										...
										<div class="d-flex">
											...
											%>
												...
												<%
												   ...
												   CachedRowSet rslike=DBConnection.selectquery("select *From like_dislike where user_id='"+ session.getAttribute("user_id") +"' and ans_id='"+ rsans.getString(1) +"'");
												   ...
												   if(rslike.next())
												   {
													...
													<a href="likedislike?x=<%= rsans.getString(1) %>&y=1&z=<%= request.getParameter("x") %>&w=zzz&u=qwe" class="like-unlink-count like">
												...
												<div class="donet-like-list">
													<a href="likedislike?x=<%= rsans.getString(1) %>&y=0&z=<%= request.getParameter("x") %>&w=zzz&u=update" class="like-unlink-count dislike">
													<i class="ri-thumb-down-fill"></i>
													    ...
													    else
													    {
	                                                        ...
	                                                        <div class="donet-like-list">
														<a href="likedislike?x=<%= rsans.getString(1) %>&y=1&z=<%= request.getParameter("x") %>&w=zzz&u=update" class="like-unlink-count like">
													...
													<div class="donet-like-list">
													<a href="likedislike?x=<%= rsans.getString(1) %>&y=0&z=<%= request.getParameter("x") %>&w=zzz&u=qwe" class="like-unlink-count dislike">
												...
												<div class="donet-like-list">
													<a href="likedislike?x=<%= rsans.getString(1) %>&y=1&z=<%= request.getParameter("x") %>" class="like-unlink-count like">
														<i class="ri-thumb-up-fill"></i>
														<span><%= rsl.size() %></span>
													</a>
												</div>
												...
												<div class="donet-like-list">
													<a href="likedislike?x=<%= rsans.getString(1) %>&y=0&z=<%= request.getParameter("x") %>" class="like-unlink-count dislike">
													<i class="ri-thumb-down-fill"></i>
														<span><%= rsdisl.size() %></span>
													</a>
												</div>
